    ...
    return (
        <nav className='border border-zinc-300 rounded-[50px] lg:h-24 md:h-20 h-14 mt-4 mx-4 px-6 flex items-center justify-between '>
            ...
            <div className='flex gap-5'>
                <button className='md:hidden relative z-[30] flex items-center' onClick={() => setMenu(!menu)}>
                    {
                        menu ? <RxCross2 className='h-6 w-6 text-black fixed right-10' /> : <RxHamburgerMenu className='h-5 w-5 text-black' />
                    }
                </button>
